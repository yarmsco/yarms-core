...
| `.claude/settings.local.json`, `.claude/scheduled_tasks.lock` | This machine's permission decisions and scheduler state; per-machine by definition, and a second machine must not inherit them. **`.claude/skills/` is versioned** — a shared skill is part of the contract, not machine state. |
...
says in words. One repo now carries the standard and its enforcement, and they version
together.

It was not always so, and the gap is worth remembering: the file that says "no accidental
local state" sat on one laptop, with no history and no second copy, for as long as it had
existed. **A standard that isn't versioned isn't a standard — it's a note.** The same test
applies to anything that joins it: if the only copy is on this machine, it is either in
the ledger above with a reason, or it is a bug to fix.
...
## Go-live runbook → the `go-live` skill

Standing up a new project, or onboarding a client? The step-by-step runbook (repo →
Supabase → model router → secrets → Railway → observability → QuickBooks → verify),
the gotchas learned the hard way, and the new-project checklist now live in
`.claude/skills/go-live/SKILL.md` — loaded when a go-live is happening instead of in
every session. Ask for the go-live runbook and it loads itself.
